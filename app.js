...
async function initTelegramApp() {
    try {
        ...
        const response = await fetch(`${API}/tg-auth`, {
            method: 'POST',const API = 'https://sahar.mist.net.ru';
    ...
    navbarButtons.forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            if (pageId === 'friends') {
                ...
                loadTasksPage();
            } else if (pageId === 'leaders') {
                // Можно добавить специальную загрузку для лидеров
            }
        });
    });
}
...
// Остальной код остаётся без изменений (как в предыдущей полной версии)
// ... [все остальные функции из предыдущего полного кода] ...
